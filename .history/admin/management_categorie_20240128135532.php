...
// Sécuriser contre les injections SQL
    $query = "INSERT INTO food (titre,price ,description, image)
    VALUES (:titre,:price ,:description, :image)";
...
?>
    ...
    <div class="main-conetent">
        <div class="wrapper">
           ...
           <button data-bs-toggle="modal" data-bs-target="#addCa"class="btn btn-primary" id="add">Ajouter catégorie</button>
           <table class="minimalistBlack text-center">
            ...
            <tbody>
            <?php
            $req = $conn->query('SELECT * FROM food');
            while($user = $req->fetch()){
                ?>
                <tr>
                <td class="image"><img class="img_cat" src="../images/<?php echo $user['image']; ?>" alt="image_card"></td>
                <td><?=$user['titre']?></td>
                <td><?=$user['price']?></td>
  ...
  <form  method="POST" enctype="multipart/form-data">
    <div class="modal-content">
      <div class="modal-header">
        <h3 class="modal-title fs-5" id="exampleModalToggleLabel">Ajouter catégorie</h3>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
      <div class="form-group">
       <input type="text" name="titre" class="form-control"  placeholder="Titre de repas" required><br>
      </div>
      <div class="form-group">
       <input type="number" name="price" class="form-control"  placeholder="Prix de service" required><br>
      </div>
      <div class="form-group">
